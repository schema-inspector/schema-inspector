...
// Validation ------------------------------------------------------------------
	...
	var _asyncValidationAttribut = {
		asyncExec: function (schema, candidate, callback) {
			var self = this;
			async.forEachSeries(Array.isArray(schema.exec) ? schema.exec : [schema.exec], function (exec, done) {
				if (typeof exec === 'function') {
					if (exec.length > 2) {
		...
		asyncProperties: function (schema, candidate, callback) {
			...
			async.series([
				function (next) {
					...
					async.forEachSeries(Object.keys(candidate), function (i, done) {
						if (i in properties) {
							return done();
						}
				...
				function (next) {
					async.forEachSeries(Object.keys(properties), function (i, done) {
						if (i === '*') {
							return done();
						}
						self._deeperObject(i);
		...
		asyncItems: function (schema, candidate, callback) {
			...
			if (Array.isArray(items) && Array.isArray(candidate)) {
				...
				async.forEachSeries(Object.keys(candidate), function (key, done) {
	...
	Validation.prototype._asyncValidate = function (schema, candidate, callback) {
		...
		async.series([
			function (next) {
				async.forEachSeries(Object.keys(_validationAttribut), function (i, done) {
			...
			function (next) {
				async.forEachSeries(Object.keys(self._custom), function (i, done) {
...
// Sanitization ----------------------------------------------------------------
	...
	var _sanitizationAttribut = {
		...
		exec: function (schema, post, callback) {
			...
			var execs = Array.isArray(schema.exec) ? schema.exec : [schema.exec];
	...
	var _asyncSanitizationAttribut = {
		asyncExec: function (schema, post, callback) {
			...
			async.forEachSeries(execs, function (exec, done) {
				if (typeof exec === 'function') {
					if (exec.length > 2) {
		...
		asyncProperties: function (schema, post, callback) {
			...
			async.series([
				function (next) {
					...
					async.forEachSeries(Object.keys(post), function (i, next) {
						if (i in properties) {
							return next();
						}
						self._deeperObject(i);
				...
				function (next) {
					async.forEachSeries(Object.keys(properties), function (i, next) {
						if (i === '*') {
							return next();
						}
						self._deeperObject(i);
		...
		asyncItems: function (schema, post, callback) {
			...
			var self = this;
			var items = schema.items;
			if (Array.isArray(items) && Array.isArray(post)) {
				var minLength = items.length < post.length
				? items.length : post.length;
				...
				async.forEachSeries(Object.keys(post), function (key, next) {
					self._deeperArray(key);
