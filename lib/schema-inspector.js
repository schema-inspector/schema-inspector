...
// Validation ------------------------------------------------------------------
	var _validationAttribut = {
		...
		globalStrict: function (schema, candidate) {
			var self = this;
			const pos = self._dumpStack();
			if (!_typeIs.array(self.dontApplyGlobalStrict)) self.dontApplyGlobalStrict = [];
			if (!_typeIs.array(self.applyGlobalStrict)) self.applyGlobalStrict = [];
			if (!candidate || !schema.properties || !_typeIs.object(candidate) || !_typeIs.object(schema.properties)) return;
			if (typeof schema.globalStrict !== 'undefined') {
				if (typeof schema.globalStrict === 'string') { schema.globalStrict = (schema.globalStrict === 'true'); }
				if (typeof schema.strict === 'string') { schema.strict = (schema.strict === 'true'); }
				if (schema.globalStrict === false) {
					self.dontApplyGlobalStrict.push(pos);
					return;
				} else {
					self.applyGlobalStrict.push(pos);
					if (typeof schema.strict === 'undefined') {
						self.strict({ properties: schema.properties, strict: true }, candidate);
					}
				}
			} else {
				var currentApplyGlobalStrict = self.applyGlobalStrict.filter(posToApplyGlobalStrict => pos.startsWith(posToApplyGlobalStrict)).sort(function (a, b) {
					return b.length - a.length;
				});
				var currentDontApplyGlobalStrict = self.dontApplyGlobalStrict.filter(posNotToApplyGlobalStrict => pos.startsWith(posNotToApplyGlobalStrict)).sort(function (a, b) {
					return b.length - a.length;
				});
				if (currentApplyGlobalStrict.length > 0 && (currentDontApplyGlobalStrict.length === 0 || currentApplyGlobalStrict[0].length > currentDontApplyGlobalStrict[0].length)) {
					if (typeof schema.strict === 'undefined') {
						self.strict({ properties: schema.properties, strict: true }, candidate);
					}
				}
	...
	Validation.prototype._validate = function (schema, candidate, callback) {
		...
		this._basicFields.forEach(function (i) {
			if ((i in schema || i === 'optional' || i === 'globalStrict') && typeof this[i] === 'function') {
			...
			function (next) {
				async.eachSeries(Object.keys(_validationAttribut), function (i, done) {
					async.nextTick(function () {
						if ((i in schema || i === 'optional' || i === 'globalStrict') && typeof self[i] === 'function') {
							if (self[i].length > 2) {
	...
	var _sanitizationAttribut = {
		...
		globalStrict: function (schema, post) {
			var self = this;
			const pos = self._dumpStack();
			if (!_typeIs.array(self.dontApplyGlobalStrict)) self.dontApplyGlobalStrict = [];
			if (!_typeIs.array(self.applyGlobalStrict)) self.applyGlobalStrict = [];
			if (!post || !schema.properties || !_typeIs.object(post) || !_typeIs.object(schema.properties)) return post;
			if (typeof schema.globalStrict !== 'undefined') {
				if (typeof schema.globalStrict === 'string') { schema.globalStrict = (schema.globalStrict === 'true'); }
				if (typeof schema.strict === 'string') { schema.strict = (schema.strict === 'true'); }
				if (schema.globalStrict === false) {
					...
					self.applyGlobalStrict.push(pos);
					if (typeof schema.strict === 'undefined') {
						return self.strict({ properties: schema.properties, strict: true }, post);
					} else {
						return post;
					}
				}
